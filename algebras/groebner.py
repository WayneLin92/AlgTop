...
class GbAlgMod2(BA.AlgebraMod2):
    ...
    def load_alg(cls, filename) -> Union[Type["GbAlgMod2"], Type["GbDga"]]:
        ...
        with open(filename, 'rb') as file:
            ...
            if init_list[0] == "5-11-2020":
                ...
                raise ValueError(f"file version not recognized: {init_list[0]}")
            # noinspection PyTypeChecker
            return type(class_name, (cls,), dct)
    ...
    def reorder_gens(cls, index_map: dict = None, key=None):
        ...
        A = GbAlgMod2.new_alg(pred=cls.pred, key=key)
        num_gens = len(cls.generators)
        rel_generators = cls.get_rel_gens()
        if index_map:
            def f(m):
                return tuple((index_map[_i], _e) for _i, _e in m)
            ...
            for i, fi in index_map.items():
                index_map_inv[fi] = i
            A.generators = [Gen(i, (gen := cls.get_gen(index_map_inv[i])).name, gen.deg, gen.deg3d)
                            for i in range(num_gens)]
        else:
            A.generators = cls.generators.copy()
        A.add_rels_data(rel_generators, clear_cache=True)
        return A
...
class GbDga(GbAlgMod2):
    ...
    # getters ----------------------------
    @classmethod
    def homology(cls, pred, BH=None, page=4, reverse=False) -> Tuple[
        Type[GbAlgMod2], dict, linalg.GradedLinearMapKMod2]:
